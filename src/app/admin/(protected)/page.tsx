...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* 1. Metric Cards Row in Pastel Accents (Image 1 & 2 reference) */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 2xl:gap-6">
        {/* Card 1: Posts Publicados (Mint Green) */}
        <div className="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 2xl:p-7 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400">Visíveis no blog</span>
            </div>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 2xl:p-7 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400">Para revisar</span>
            </div>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 2xl:p-7 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400">No storage</span>
            </div>
          </div>
        </div>
      </div>

      {/* 2. Interactive Editorial Calendar (Image 1 & 3 reference) */}
      <EditorialCalendar posts={posts} />

      {/* 3. Bottom Row: Categories Donut Chart + Recent Activity List (Image 2 & 3 reference) */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 2xl:gap-8">
        {/* Left Column: Distribuição por Categoria (Donut Chart - Image 2) */}
        <div className="rounded-3xl border border-slate-100 bg-white p-6 sm:p-8 shadow-sm lg:col-span-5 2xl:col-span-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Distribuição das publicações na plataforma
                </p>
              </div>
              <Link
          ...
          <div className="mt-6 rounded-2xl bg-emerald-50/60 p-4 border border-emerald-100">
            ...
            <p className="mt-0.5 text-[11px] text-emerald-600">
              Estes artigos aparecem com ênfase visual na listagem do blog.
            </p>
          </div>
        </div>

        {/* Right Column: Últimos Posts / Atividades Recentes (Image 3 reference) */}
        <div className="rounded-3xl border border-slate-100 bg-white p-6 sm:p-8 shadow-sm lg:col-span-7 2xl:col-span-8 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg font-bold tracking-tight text-slate-900">
              ...
              {posts.length > 0 ? (
                posts.slice(0, 5).map((post, idx) => {
                  ...
                  return (
                    <div
                      key={post.id}
                      className="group flex items-center justify-between py-3.5 transition-colors hover:bg-slate-50/60 rounded-xl px-2 -mx-2"
                    >
                      <div className="flex items-center gap-3.5 min-w-0">
                        {/* Soft colored category icon (Image 3 reference) */}
                        <div
                          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${color.bg} ${color.text} shadow-2xs`}
                        >
                          <FileCheck2 className="h-5 w-5" />
                        </div>
